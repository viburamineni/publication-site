...
describe('homepage-controls Contentful migration toolchain', () => {
  it('invokes the repository-installed migrator with the exact target and arguments', () => {
    const migrationCommand = manifest.scripts['contentful:migrate:homepage-controls'];

    expect(migrationCommand).toBe(
      'contentful-migration --space-id "$CONTENTFUL_SPACE_ID" --environment-id "${CONTENTFUL_ENVIRONMENT:-master}" --access-token "$CONTENTFUL_MANAGEMENT_TOKEN" --yes contentful/migrations/006-clean-homepage-controls.cjs',
    );

...
describe('initial Contentful migration toolchain', () => {
  it('invokes the repository-installed migrator without ad hoc package execution', () => {
    const migrationCommand = manifest.scripts['contentful:migrate'];

    expect(migrationCommand).toBe(
      'contentful-migration --space-id "$CONTENTFUL_SPACE_ID" --environment-id "${CONTENTFUL_ENVIRONMENT:-master}" --access-token "$CONTENTFUL_MANAGEMENT_TOKEN" --yes contentful/migrations/001-initial-model.cjs',
    );
